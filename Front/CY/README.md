...
프로젝트 구조,
개발 과정,
프로젝트 시연을 마치고 트러블 슈팅과 프로젝트 후기에 대해 
말씀드리겠습니다.

먼저 프로젝트 소개입니다. 
저희가 구현한 프로그램은 
기본적인 학사관리와 함께 코로나 비대면 시대에 맞춰 인터넷 강의 및 시험을 제공하는 프로젝트 입니다.
----------------------------------------------
다음은 업무 분장입니다.

먼저 유성열 PM은 전체적인 프로젝트를 총괄하면서 
작업 환경을 구축하였고 클래스를 설계하였습니다.

또한 노상문 PL을 중심으로 Back-End팀은 프로시저 제작, Java DB연동, DB페이지 연동, DB서버를 관리 하였습니다.

Front-End 팀은  오세현 PL을 중심으로
부트 스트랩을 이용해 UI를 구현하고 Front  설계 및 조립을 하였고
안드로이드를 구현했습니다.
---------------------------------
다음은 적용기술 소개입니다.

Front에서는 Figma를 이용해 UI목업 작업을 진행 하였고
 툴로는 VSCODE이용하였습니다.
사용한 기술로는 HTML, CSS, JAVASCRIPT, 부트스트랩
,jQuery , Ajax 등을 이용하였습니다. 
 
백앤드는 툴로는 Toad  DataGript, 인텔리 제이
을 이용하였고
사용한 기술로는 프로시저, mybatis, Tomcat을 사용하여 Servlet을 이용했습니다.

공통적으로 깃허브 속에 있는 WIKI 그리고 노션을 이용해 PROJECT에 대한 문서관리를 하였고
깃을 이용해 형상관리 를 하였습니다.
------------
다음은 프로젝트 구조입니다.

저희 프로그램은 부트에 내장된 톰캣 서버가 
브라우저에서 들어온 요청을 받고, 
디스패처서블릿을 통해 
각 컨트롤러로 전달되어 로직을 타고 DB와 연동되는 구조입니다. 

------------
다음은 개발 과정입니다.

-Figma-
저희는 Figma를 통해 UI목업 뿐만 아니라 각 화면에 필요한 프로시저명을 기록했습니다.


-Git History-
저희는 유성열 PM을 필두로 각자 작업을  일정 시기마다 머지
를 하였습니다. 또 한 COMMIT시 메시지를 최대한 상세하게 적어
충돌 시 발생하는 문제들을 방지하였습니다.

-GitHub Wiki-
저희는 GitHub Wiki에서 주간 회고록, 그라운드룰, 명세서 등 
다양한 문서를 관리 했습니다.

-GitHub GroundRule - 

저희는 프로젝트를 진행할 때 공통의 목표를 위해 
꼭 필요한 사항을 룰로 만들어 정리 했습니다.

- 주간 회고록 -
 각자 한주가 끝나면 주간회고록에 
그주에 대한 느낀점 아니면 필요사항을 남겼습니다.


저희는 노션으로도 문서이력관리를 했습니다.

- Notion  카테고리 - 
 노션 카테고리를 보시면 메뉴얼 , 회의록 등등 문서와 이력관리를 하였습니다.

- 노션 출석부 -
또 한 저희는 노션 출석부를 통하여 매일매일 후기를 남겼습니다.

- ERD -

다음은 ERD입니다.
저희는 학사 관리 프로그램인 만큼 학생을 중심에 두고 ERD를 그려나갔습니다.

- 클래스 다이어 그램 -

다음은 클래스 다이어그램 입니다 .

대표적으로 Admin파트를  보여드리겠습니다.

- 프로시저 정의서 -

다음은 프로시저 정의서입니다. 
대표적으로 UPDATE_MEMO와 COMBOBOX를 보여드리겠습니다.

- 요구 사항 명세서 - 

다음은 요구사항 명세서입니다 . 

 기본기능과 종합정보조회 강좌 조회에 대한 요구사항  명세서 입니다.

다음은 등록금 조회, 예약 시스템, 시간표 시스템에 대한 업무정의서입니다.

다음은 인터넷강의 업무 정의서 설명입니다..

네 다음은 프로젝트를 시연해보겠습니다.

시연은 현재 완성된 기능들 위주로 보여드리겠습니다.
- 시연 --

1. 로그인 페이지 => 로그인 페이지이며 로그인을 하는순간 학생의 학번이 세션에 저장이 되어 메인페이지로 넘어갑니다.

2. 메인 페이지 => 메인페이지이며 제일 상위 항목 Pprk.Main() 페이지는 로그인한 학생의 시간표확인과 메모를 저장할 수 있는 페이지입니다.

3. 다음으로 대분류 종합정보조회입니다. 

4. 신상정보조회입니다. 신상정보에서 학생에 영문성명, 연락처, 이메일, 보호자 성명, 보호자 연락처를 수정할 수 있습니다.

5. 수강목록조회입니다. 수강목록조회를 누르면 콤보박스에 해당학생이 등록한 년도,학기가 저장을 하며 해당학기를 선택 시
      수강 목록을 조회할 수 있습니다. 

6. 수강성적조회입니다. 수강성적조회를 누르는순간  수강목록 테이블에 수강목록이 나타납니다.

7. 해당학기의 성적 상세조회를 위해 수강목록의 행을 클릭을 하게 되면 당학기의 교과목별 성적 상세조회가 나타납니다.

8. 학적변동조회입니다. 학적변동조회를 누르는순간  로그인한 학생의 학적변동내역이 나타납니다.

9. 다음은 강좌조회 입니다.

10. 개설강좌조회입니다. 대학 학부 학과의 콤보박스를 누르게 되면 각각 누를때마다 프로시저를 타게되어 대분류 중분류 소분류 같은 느낌으로 데이터를 보여줍니다.

11. 커리큘럼 조회입니다. 수강목록조회와 같이 콤보박스에 데이터가 먼저 들어가고 콤보박스에서 년도를 누르는순간 해당년도의 커리큘럼이 나타납니다.

12. 다음은 등록금 조회입니다. 콤보박스에서 학기를 누르면 해당 학기에 등록금이 테이블에 보여집니다.

13. 다음은 E-Learning 입니다. 강좌를 누르면 해당 강좌에 강의 목록이 나오고
강의 셀을 누르면 강의영상이 나옵니다. 파일 첨부를 누른 후 과제를 제출 할 수 있습니다. 피드백보내기를 누르면 피드백 내용을 작성 후 피드백을 보낼 수 있습니다.

13-1 다음은 시험 응시입니다 시험응시를 누르면 해당학생에 해당하는 
시험 문제들이 나오고 정답들을 선택 후 완료를 누르면 제출 됩니다.

14. 다음은 관리자 페이지입니다. 
학생, 교직원조회에서 콤보박스를 누른후 검색을 누르면 콤보박스 조건에
해당하는 학생/교직원이 나오고 셀을 클릭하면 상세 정보가 조회됩니다.

15. 학생, 교직원 추가에서는 해당하는 값들을 입력하고 추가할 수 있습니다.

16. 학생, 교직원 수정에서는 해당하는 값들을 수정 할 수 있습니다.



-안드로이드-
다음은 안드로이드 시연 시작하겠습니다.
전체 구조는 메인 액티비티 하나에 프래그먼트 여러개를 사용했습니다.

로그인 버튼을 누르면 홈 프레그먼트로 이동합니다.

홈 프레그먼트에 시간표, 강좌목록 조회, 대학 홈페이지, 포털사이트 메뉴가 있습니다.

먼저 시간표 부터 보여드리겠습니다. 
시간표로가면 해당 학생에 시간표를 보여줍니다.

강좌 목록입니다. 강좌목록을 누르면 
해당학생이 신청한 온라인 강좌목록이 뜹니다.
여기서 강좌를 누르면 해당강좌에 강의 목록이 나옵니다.

대학홈페이지와 포털사이트를 누르면 해당하는 링크로 이동합니다.
...
------------
다음은 트러블 슈팅입니다.
저희는 각자의 트러블 슈팅과 서로의 코드리뷰를 GitHub - Issue페이지에서
관리를 했습니다.
-------------
(3번) 필드 주입 이슈부터 시작하겠습니다.

이클립스 에서는 경고가 표시되지 않지만,
IntelliJ 사용시 해당 프로젝트에서 Field Injection 사용을 위한 @Autowired에 경고가 나타나는 이슈가 올라왔습니다.

IntelliJ 내부에서 나타내는 경고 문구를 확인해보고 
필드 주입법에서 생성자 주입법으로 변경을 했습니다. 다음

필드 주입법에서 생성자 주입법으로 바꾼 이유는
순환 참조가 발생시 스프링은 어떤 빈을 먼저 어떻게 생성해야할지
결정을 못 한다는 에러를 나타내주고 실행이 되지 않습니다.

하지만 필드 주입시 해당 에러를 판단하지 못하고 실행하게 되어
실제 코드 호출 전까지는 문제를 발견 할 수 없습니다. 다음

그렇기에 생성자 주입법을 사용하면..
오류가 방지되고 테스트에 용이하며 코드도 깔끔하게 정리할 수 있습니다.
------------------
(5번)
다음은 시간표 이슈입니다.
다음 쿼리문을 이용해 디비에 있는 과목을 불렀지만 
다음 문제들이 적용이 되었습니다.

저희는 이 문제를 인지하고 DML문을 다음과 같이 수정한후 다음

ROWSPAN이 적용되는 코드를 넣어서 DB에 연동하여 해결하였습니다.
---------------
(6번)
다음은 프로시저에 대한 이슈입니다.

해당 강의에 해당 학생이 제출한 과제가 있을 때 문제 없이 조회되는 모습입니다. 다음

하지만 해당 강의에 해당 학생이 과제를 제출하지 않았으면 HOMEWORK 테이블에 값이 없기에
쿼리문이 전체적으로 조회되지 않습니다. 다음


저희는 커서를 두개로 분리하고 한 프로시저에서 커서를 두개 받는 걸로 해결하였습니다.

(안드로이드 트러블 슈팅)
다음은 안드로이드 이슈입니다.    다음

WebView를 사용하여 URL에 접속 시 인터넷 창이 앱 내부에서
실행 되지 않고 새창으로 나옵니다.

왜냐하면 기본 WebView 설정은 새로운 URL 접속 시 시스템에서
새로운 창에 로딩이 되기 때문입니다.  다음

해서 다음과 같이 WebViewClient를 WebView 내부에서 
생성하여 해결 하였습니다. 다음


추가로 경로가 http로 바뀌어서 접속이 된 후  에러가 뜨는 상황에서
AndroidMainifest에서 
android:usesCleartextTraffic="true" 를
추가해서 모든 http URL의 접근을 허가를 시켜주면 
정상적으로 작동합니다.
...
각자 느낀게 다르지만 각자 맡은 업무에 최선을 다한거 같습니다.
...
오전에 소스 좀 올려 놨는데 일단 메인 액티비티 쪽만.
직렬화 문제가 좀 그것만 추가하면 될 줄 알았더니 추가로 연결되는 부분이 있어서요.
여기 그레이들에서 일단 좀 제보해주시죠.
오늘 다 구현하려는 게 아니라 단위테스트라도 해보고 싶어서 오후까지 해보고 있는데 방해가 되니까 제거해 주시고요.
그 다음에 북쪽에 가서도 거둬 주세요.
어노테이션과 앞부터 튀까지
제거 하신 다음에는 반드시 싱크나우 눌러주셔야 하고요.
그 다음에 단위테스트부터 해보실 거면 일단은..
레트로핏 같은 경우도 타는지 안타느지 확인해 본 적이 없어서.
확인을 좀 해봤으면 좋겠으니까 s를 빼볼게요.
일부로 터뜨려보는 거니까ㅣ.
여기에 미시지가 찍혀야 겠죠
여기에 내가 구성한 메소드가 정상이라면 터저야 하거든요.
처음 써보는 애들이라서.
그러면 일단 서비스 쪽에서 확인해야 할 것은 인터페이스 인 것이 사실이고
이쪽에 아래쪽으로 내려와서 api베스트 셀러 제이슨 앤퍼센트 연산자 카테고리 아이디 백번.
그다음에 함수이름.
겟데이터셀러북스
apkstring
된 것 같거든요
...
인터넷 커미션 추가해 주세요.
매니 페스트 파일에다가.
여기 보시면 퍼미션 디나이 미신 인터넷 퍼미션 이렇게 되어 있는거 보이시죠?
추가가 안되면 여기서 에러메시지 나오게 띄어논 게 이거거드뇽.
이쪽에 가서 그 부분은 반드시 선 처리가 되어야 할 것 같고요.
일단은 이거 자체가 레트로 핏에대한 검증이 된 것이 아니기 때문에 다시한번 이거를 추가하고 저장하신 다음에.
지금 https라고 써야 하는데 http까지 밖에 안적었기 때문에 문제가 되어야 할 거예요.
다시 해볼게요.
에러 등급에 맞춰져 있어서.
여기 보시면 네트워크 시큐리티 폴릿이라고 나오시는거 보이시죠?
근데 어차피 우리가 포스트맨쪽에 보면 이쪽에 사진이 걸려 있는데가 다 http야.
이 문제는 거기를s로 붙이는 건데
장기적으로 볼때는..
차라리 추가해 주시는게 좋을 것 같아요.
그 부분 반영해 주시고요.
에러 메시지 확인 했으면 다시 말아주시고.
기존에 실행했던건 일단 껏다가 다시 실행해 주세요
언노운 서비스 익셉션
근데 액티비티 이쪽ㅇ느 s를 붙이는게 낫겠죠?
없어도 되지만 실제 지원이 되는 사이트니까.
다만 이미지 같은 경우는 예외니까.
보시면 그 로그 쪽에 그 정보가 정상적으로 출력 되는거 보이시죠.
조립이 잘 됐다는걸 확인 할 수 있었ㄴ[요
api보고 대략 맞춰봤는데 json포맷으로 받아서 로그에 찍어 봤으니까
우리가 뷰쪽에 붙이기 전에 일단 타이틀이라도 리스트아답터 붙여서 써보고 나머지 도서이미지라든가 추가로 더 보고싶다면 따로 작업을 하셔야 할 것 같고
나머지는 도서 아이디니까 이걸 가지고 식별하면 될 것 같아요.
자 여기까지 해서 마무리 했고요.
이 다음 버전은 내일또 하기로 하고 이남은 시간은 스크립트 작업을 해 볼게요.
일단 여기까지 마무리좀 해줘요
시간 한5분 기다려 보고 나머지는 스크립트까지는 오늘 소개를 해야 혼자 해볼사람들이 해볼 수가 있을것 같아서 진도는 오늘 더 안나가려고요.
끝난게 아니라 다음주까지 해볼거니까.
이거 가지도 db연동도 하고 히스토리 관리도 하고 검색, 조건검색도 처리를 하고 해보자고요.

---

NullSaft 한 코드를 사용하기 위해서 non-null Type으로 변수를 선언함

초기 값이 없는 변수는 어떻게 초기화를 해야할까?
초기 값이 없으면 변수 선언 자체가 안되는데...
...
a  = null; //wrapper class  => 메소드도 호출이 가능하다.
a.sum(); // NullPointerException
...
spring책 143페이지 ,145페이지

`con.setAutocommit(true);`

`con.setAutocommit(false); SQLException -> DataAccessException`
...
`delete():1`
`insert():1`
`if(r1==1 && r2==1 && r3==1) con.commit();`
`else con.rollback()`

---

책 157페이지 => spring-service 4-1-1
Logic클래스에 있는 모든 메소드는 pointcut이다. 바깥쪽은 조인 포인트 안쪽은 pointcut 그안에 정규식을 쓸거다.

`<aop:config>`
`<aop:pointcut expression="execution(* web.mvc.*Logic.*(..))" id="point-cut"/>` ==>Logic다음 괄호에 ..은 메소드가 들어간다.
`<aop:advisor advice-ref="tx-advice" pointcut-ref="point-cut"/>`=>advisor(감시자)가 pointcut을 감시한다.
`</aop:config>`

`(* web.mvc.*Logic.\*(..))"` ==> xml에 적을 때는 풀네임을 적어야한다.

`<tx:advice id="tx-advice" transaction-manager="transactionManager">`==> 스프링에서 제공한다. 그래야 인터셉트하고 트랜잭션처리를 해준다.

`<tx:attributes>`
`<tx:method name="do*"/>` ==> @으로 바뀐다.
`<tx:method name="cud*"/>`
`</tx:attributes>`

eud xxx => 바로커밋 책229페이지 234페이지
...
안녕하십니까 4조 팀 파프리카 크루원 강찬영입니다 반갑습니다!
...
업무정의서,
프로젝트 시연을 하고,
프로젝트 시연이 끝나면 트러블 슈팅과 후기에 대해 말씀드리겠습니다.

먼저 프로젝트를 소개해드리겟습니다.

주제선정 배경(2분)

조가 정해지고 첫 회의를 통해서 주제를 정하게 되었습니다.

회계시스템, 택배시스템, 렌탈시스템, 중고차 거래 사이트, 사용자경험공유사이트, 스포츠경기분석사이트 등 여러가지 주제가 나왔고

회의를 통해 회계시스템을 포함한 회사 그룹웨어가 결정이 되었었습니다.

하지만 설계단계에서 회계에 대한 구현에 발목이 잡혀 구현단계까지 계획한 기간내에 될거 같지 않아 기존 주제를 포기하고

6월7일날 기존 주제와 비슷한 학사시스템으로 주제를 바꿔 초기설계부터 다시 시작을 했습니다.

다음 공정표를 소개해드리겠습니다.

공정표 소개(1분)

이건 최초 공정표인데 앞에서 말씀드렸다시피 중간에 주제를 바꾸게되어 처음에 저희는 회계관리 프로그램을 목표로 설계단계까지 진행하였지만 생각보다 복잡한 회계시스템을 구현하기에는
기간내에 어려울거 같아
다시 설계단계부터 준비를 하게 되었고 1차 발표용 ERD는 21일 클래스 다이어그램22일 프로시저는 23일 UI 페이지를 디비랑 연결하는 작업은 7월 1일에 끝마쳤습니다.

그 후 2, 3차때는 나머지 남은 기능들을 목표로 구현을 할 예정입니다. //여쭤보기

다음은 적용기술 소개입니다.

적용기술 소개(1분)

저희는 Figma를 사용하여 UI목업을 만들고 진행과 프로시저 변수와 컬럼명을 정하였습니다

깃허브 속에 있는 WIKI 그리고 노션을 이용해 PROJECT에 대한 문서관리를 하였고

깃을 이용해 형상관리 등 여러가지 문서작업을 했습니다. 를 하였습니다.

프론트는 툴로는 VSCODE, ECLIPSE를 이용하였고, 사용한 기술로는 HTML, CSS, JS , JSP, jQuery, AJAX, BOOTSTRAP 등을 이용하였고

백앤드는 툴로는 X, X, X를 이용하였고, 사용한 기술로는 X X X XX X X X 가 있습니다.
Toad, DATA GRIP을 이용해서 ORACLE DB서버에 테이블 생성 및 데이터 입력 , 프로시저 작성 , 시퀀스등 데이터 튜닝등을 진행하였고 자바에서 컨트롤러와 로직 다오를 작성하여 연결하고 페이지에 해당 기능에 맞는 프로시저를 호출하여 페이지에 값을 전달하는 작업을 진행하였습니다.

다음은 파트 구성입니다.

파트 구성(1분)

유성열 PM은 프로젝트를 전체적으로 총괄하였으며 DB,ERD,프로시저 제작을 맡았으며 컨트롤러 코딩을 하였습니다.

오세현 PL은 부트스트랩을 이용한 UI 구현 및 JS, JQEURY, AJAX를 통한 UX 설계 및 프론트 설계조립을 담당했습니다.

노상문 PL은 ERD, 프로시저 제작을 하였으며 JAVA DB를 연동하였으며 CRUD-StudentInfo를 담당하였습니다.

장해리 CREW는 ERD,프로시저 제작을 하였으며 CRUD-Curriculum를 담당하였습니다.

최진규 CREW는 ERD, 프로시저 제작 및 CRUD-Course, CRUD-OnlineTest 제작을 담당하였습니다.

저, 강찬영 CREW느 FRONT CREW으로서 부트스트랩을 이용한 페이지 구현 및 CRUD-Schedule 그리고 1차 프로젝트 발표를 맡게 되었습니다.

박정호 CREW는 FRONT CREW로서 부트스트랩을 이용한 페이지 구현 및 CRUD-Schedule, Ppt 제작을 했습니다.

다음으로 업무정의서를 설명해드리겠습니다.

업무정의서 설명(5분) (<<<2분)

저희는 수많은 회의를 통해 전체적인 프로세스중 중요한내용을 토대로 업무정의서를 작성하였습니다.

다음은 기본기능과 종합정보조회 업무 정의서입니다. (15초 기다린다..)

다음은 강좌 조회 , 등록금 조회 , 시간표 시스템 업무 정의서입니다..(15초 기다린다..)

다음은 인터넷강의 업무 정의서 설명입니다..(15초 기다린다..)
...
1. 로그인 페이지 => 로그인 페이지이며 로그인을 하는순간 프로시저가 실행되어 학생의 학번이 세션에 저장이 되어 메인페이지로 넘어갑니다.

2. 메인 페이지 => 메인페이지이며 제일 상위 항목 Pprk.Main() 페이지는 로그인한 학생의 시간표확인과 메모를 저장할 수 있는 페이지입니다.

3. 다음으로 종합정보조회입니다. 이 대분류 안에는 로그인한 학생의 기본적인 학사정보를 조회할 수 있습니다.

4. 신상정보조회입니다. 옆 사이드바에서 신상정보조회를 누르는순간 프로시저가 실행이 되어 데이터를 페이지에 저장하고 ajax를 통해 오른쪽 페이지가 바뀌는순간 데이터를 뿌려줍니다.

5. 수강목록조회입니다. 수강목록조회를 누르는순간 프로시저가 실행되어 콤보박스에 해당학생이 등록한 년도,학기가 저장이 됩니다.

6. 콤봑스를 조작하여 바꾸는순간 프로시저가 실행되어 해당 년도,학기에 수강한 강좌들을 json 형식으로 가져와 table에 뿌려줍니다.

7. 수강성적조회입니다. 수강성적조회를 누르는순간 프로시저가 실행되어 수강목록 테이블에 수강목록이 뿌려집니다.

8. 해당학기의 성적 상세조회를 위해 수강목록의 행을 클릭을 하게 되면 프로시저가 실행이 되어 당학기의 교과목별 성적 상세조회가 테이블에 뿌려집니다.

9. 학적변동조회입니다. 학적변동조회를 누르는순간 프로시저가 실행되어 로그인한 학생의 학적변동내역이 테이블에 뿌려집니다.

10. 대분류 강좌조회입니다.

11. 개설강좌조회입니다. 대학 학부 학과의 콤보박스를 누르게 되면 각각 누를때마다 프로시저를 타게되어 대분류 중분류 소분류 같은 느낌으로 데이터를 가져와서 뿌려주게 됩니다.

12. 모든 데이터를 고른뒤 검색버튼을 누르면 프로시저가 시행되어 해당하는 데이터를 표에 뿌려줍니다.

13. 커리큘럼 조회입니다. 수강목록조회와 같이 콤보박스에 데이터가 먼저 들어가고 콤보박스에서 년도를 누르는순간 해당년도의 커리큘럼이 테이블에 뿌려지게 됩니다.

14. 현재까지가 1차발표 전까지 완성이 된 내용입니다.
...
1차 개인 후기 또는 전체 후기(1분)

1차 프로젝트 발표 후기 : 많이 부족한 상태에서 참여를 했지만 팀원들이 잘 이끌어주고 업무를 잘 분담해주셔서 즐거웠습니다.

트러블 슈팅

내 브랜치에 작업한 파일이 세현님도 작업을 하셨고 내꺼는 쓰레기 파일이기에 commit때 올리지 말았어야 했지만 그대로 넣고 푸쉬를 넣은 상태에서
성열이형껄 pull을 떙기니 컴플리트가 발생하였다

`λ cd D:\final_project\KOSMO80_FINAL`
`λ git fetch`
`λ git checkout dev`
`λ git pull`
`λ git merge --no-ff dev` ==>컴플리트 부분
밑부터 해결
`λ git mergetool` ==> 여기서 컴플리트 난 부분을 수정하였다! 컴플리트가 발생해도 에러난 부분을 차근차근 확인하여 바꾸면된다.
`λ git merge --continue`
`λ git hist` ==> hist를 자주 확인하여 나의 위치를 파악하자!

# 21/06/30

파이널 프로젝트 1차 발표 대본

안녕하십니까 4조 파프리카 1차 발표자 강찬영입니다 반갑습니다!

먼저 저희 발표 순서를 말씀 드리겠습니다.
...
처음 조가 정해진날 저희는 회의를 통해서 주제를 정하게 되었습니다. ERP, ERD를 이용한 회계시스템, 전산이 종합적으로 들어간 택배시스템이나 렌탈시스템
또는 중고차 거래 사이트 등 여러가지 주제가 나왔고 ERP, ERD를 이용한 회계시스템을 구현하도록 결정하였습니다.
하지만 설계단계에서 진행이 늦어졌고 구현단계까지 계획한 기간내에 될거 같지 않아 6월7일날 여러 주제를 제시하고 회의 끝에 가장
친숙하고 각자 만들고 싶은 부분을 쉽게 추가할 수 있는 학사 ERP를 선택했습니다.

공정표를 소개해드리겠습니다.

공정표 소개(1분)

이건 최초 공정표인데 앞에서 말씀드렸다시피 처음에 저희는 회계관리 프로그램을 목표로 설계단계까지 진행하였지만
생각보다 복잡한 회계시스템을 구현하기에는 기간내에 어려울거 같아 다시 설계단계부터 준비를 하게 되었고
ERD는 21일 클래스 다이어그램22일 프로시저는 23일 UI 페이지를 디비랑 연결하는 작업은 7월1일날 마쳤습니다.
그 후 2, 3차때 앱 및 .. 을 목표로 구현을 할 예정입니다. //여쭤보기

다음은 적용기술 소개입니다.

적용기술 소개(1분)

저희는 Figma 툴을 사용하여 UI목업 진행과 프로시저 변수와 컬럼명을 정하였습니다

노션을 이용해 회의록 작성, 출결관리 등 여러가지 문서작업을 했습니다.

프론트는 기본적으로 VS Code를 이용해 HTML,CSS, JS 그리고
부트스트랩 이용해 틀을 만들었고 jQuery, xml, ajax를 이용해 구체적인 기능을 구현하였습니다.

백앤드는 Toad, DATA GRIP을 이용해서 ORACLE DB서버에 테이블 생성 및 데이터 입력 , 프로시저 작성 , 시퀀스등 데이터 튜닝등을
진행하였고 자바에서 컨트롤러와 로직 다오를 작성하여 연결하고 페이지에 해당 기능에 맞는 프로시저를 호출하여 페이지에
값을 전달하는 작업을 진행하였습니다.

다음은 파트 구성입니다.

파트 구성(1분)
유성열님은 PM으로서 프로젝트를 전체적으로 총괄하였으며 DB,ERD,프로시저 제작을 맡았으며 컨트롤러 코딩을 하였습니다.
오세현님은 FRONT PL로서 UI MAPPING을 맡았으며 웹페이지 제작을 총괄하였습니다.
노상문님은 BACK PL로서 ERD, 프로시저 제작을 하였으며 JAVA DB를 연동하였으며 CRUD-StudentInfo를 담당하였습니다.
장해리님은 BACK CREW원으로서 ERD,프로시저 제작을 하였으며 CRUD-Curriculum를 담당하였습니다.
최진규님은 ERD, 프로시저 제작 및 CRUD-Course, CRUD-OnlineTest 제작을 담당하였습니다.
저는 FRONT CREW으로서 부트스트랩을 이용한 페이지 구현 및 CRUD-Schedule 그리고 1차 프로젝트 발표를 맡게 되었습니다.
박정호님은 FRONT CREW로서 부트스트랩을 이용한 페이지 구현 및 CRUD-Schedule, Ppt 제작을 했습니다.
...
`<uses-permission android:name="android.permission.INTERNET"/>` <!--인터넷 연결 옵션-->
`android:usesCleartextTraffic="true"` <!--http 옵션-->
